    ...
    <div class="mx-auto px-6 py-10 max-w-7xl">
        <div class="w-full mx-auto px-0 pt-0 md:pt-5 pb-6 mt-4 border-0 md:border border-gray-300 rounded-md bg-transparent md:bg-white md:px-6 sm:mt-8 sm:mb-5 sm:w-10/12 md:w-11/12">
            ...
            <div class="flex justify-between relative">
                <h2 class="text-lg">Deposits</h2>

                <button id="deposits-history-button">
                    <div class="h-4 w-4">
                        <img src="{{ asset('images/downarrow.svg') }}">
                    </div>
                </button>

                <div id="deposits-history-menu" class="absolute bg-white hidden flex-col rounded top-0 right-0 bottom-auto left-auto m-0 -translate-x-[20px] translate-y-[20px] border-[1px] border-solid border-gray-200 w-24 p-2 z-10 ">
...
<script>
    const Dhistorybutton = document.getElementById('deposits-history-button');
    const Dhistorymenu = document.getElementById('deposits-history-menu');
    Dhistorybutton.addEventListener('click', ()=>{
        Dhistorymenu.classList.toggle('hidden');
    });
